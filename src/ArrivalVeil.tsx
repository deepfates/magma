...
import {Check, Copy, Flame, LockKeyhole, WifiOff, X} from 'lucide-react';
import './arrival.css';
...
const phaseCopy: Record<ArrivalPhase, string> = {
  floor: 'People are here. Entering will not change the clock or the view.',
  gathering: 'The porch is open. Come in whenever you’re ready.',
  porch: 'The porch is open. Come in whenever you’re ready.',
  unknown: 'Entering will not change the shared clock, window, or anyone else’s sound.',
};
      ...
      >
        ...
        {state.kind === 'creating' && state.step === 'form' && (
          <form aria-busy={state.submitting || undefined} onSubmit={submitCreate}>
            <p className="arrival-kicker">A shared place</p>
            <h2 id={titleId} tabIndex={-1} data-initial-focus>Make a porch</h2>
            <p id={descriptionId}>A window onto the world that friends can draw on, leave things on, and return to.</p>
            ...
            <button className="arrival-primary" type="submit" disabled={state.submitting || !displayName.trim() || !emoji.trim()}>{state.submitting ? 'Opening porch…' : 'Open the porch'}</button>
        ...
        {state.kind === 'creating' && state.step === 'ready' && (
          <div>
            ...
            <h2 id={titleId} tabIndex={-1} data-initial-focus>Your porch is ready.</h2>
            <p id={descriptionId}>Invite someone, or go in and leave the first mark.</p>
            <div className="arrival-actions arrival-actions-stack">
              ...
              <button className="arrival-secondary" type="button" onClick={onEnterCreatedRoom}>{state.invitationPayload ? 'Enter the porch' : 'Go in alone'}</button>
            </div>
        ...
        {(state.kind === 'invited' || state.kind === 'returning') && (
          <form aria-busy={state.entering || undefined} onSubmit={submitEntry}>
            <p className="arrival-kicker">{state.kind === 'returning' ? 'Welcome back' : state.inviterName ? `${state.inviterName} invited you` : 'You’re invited'}</p>
            ...
            <div className="arrival-actions">
              <button className="arrival-primary" type="submit" disabled={state.entering || !displayName.trim() || !emoji.trim() || (state.kind === 'invited' && state.invitationRequired && !invitationCode.trim())}>{state.entering ? 'Entering…' : 'Come in'}</button>
        ...
        {state.kind === 'proving' && (
          <div aria-busy="true">
            <p className="arrival-kicker">The Porch</p>
            <h2 id={titleId} tabIndex={-1} data-initial-focus>Opening the porch…</h2>
