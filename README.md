...
The current deployed slice is a signed, revocable private focus room with an exercised first Floor → Porch → Floor loop, not yet the complete social salon defined for 1.0 above. The server-authoritative listening queue, bounded paginated communication and moderation history, real SFU/TURN voice, finer-grained room policy, operational nuisance limits, and representative human salon evidence remain incomplete.
